...
makeTSCompare func = newFunc
 where
  newFunc first@(i, Nothing) (       _, Nothing) = first
  newFunc (      _, Nothing) second@(i, val    ) = second
  newFunc first@(i, val    ) (       _, Nothing) = first
  newFunc first@(i1, Just val1) second@(i2, Just val2) =
    if func val1 val2 == val1 then first else second
...
movingAverageTS empty@(TS []    []    ) _ = empty
